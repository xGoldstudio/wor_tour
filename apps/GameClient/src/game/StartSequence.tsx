...
export default function StartSequence() {
  ...
  useGameEventListener({
    ...
    action: () => {
			...
			const element = wrapperRef.current?.querySelector(".startText") as HTMLElement;
      registerAnimation({
				duration: 240,
        computeStyle: animationTimeline(240).add(
          ...
          [
            ...
            { values: { scaleX: 1 }, from: 50, ease: [0, 0.42, 1, 1] },
						...
						{ values: { scaleX: 1.05 }, from: 80, ease: [0, 1, 1, 1], onStart: () => element.innerHTML = "2" },
            { values: { scaleX: 0.85 }, from: 90, ease: [0, 1, 1, 1] },
            { values: { scaleX: 1 }, from: 110, ease: [0, 0.42, 1, 1] },
						
						{ values: { scaleX: 1.05 }, from: 140, ease: [0, 1, 1, 1], onStart: () => element.innerHTML = "1" },
            { values: { scaleX: 0.85 }, from: 150, ease: [0, 1, 1, 1] },
            { values: { scaleX: 1 }, from: 170, ease: [0, 0.42, 1, 1] },

						{ values: { scaleX: 1.05 }, from: 200, ease: [0, 1, 1, 1], onStart: () => element.innerHTML = "Start!" },
  ...
  return (
    <div
      className={cn("left-0 top-0 absolute w-full h-full pointer-events-none z-20 flex justify-center items-center", isStarted && "hidden")}
      ref={wrapperRef}
    >
      <div className="absolute top-0 left-0 w-full h-full bg-black bg-opacity-30"></div>
      <p className="startText text-[150px] text-white relative drop-shadow-[2px_2px_2px_black]">3</p>
    </div>
  );
}
