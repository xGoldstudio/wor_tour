...
export function DeckCardUI({
  ...
  function onDragEnd() {
    setReplacingCard(replacingCard => {
      if (replacingCard === true) {
        return null;
      }
  ...
  return (
    <Draggable
      disabled={(!replacingCard || !deckCard) && !scrollable}
      dragData={card.id}
      dragRef={dragRef}
      onDragEnd={onDragEnd}
    >
      ...
      >
        ...
        <Button
          innerRef={wrapperRef}
          unstyled
          dontPreventPropagation={!replacingCard}
          onLongPressAction={
            deckCard
              ? (e) => {
                  if (!replacingCard) {
          ...
          action={() => {
            ...
            if (replacingCard || isSelectable === false) return;
