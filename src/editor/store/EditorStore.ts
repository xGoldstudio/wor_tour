...
const useEditorStore = create<EditorStore>()((set, get) => ({
	...
	getCardsByWorld: (world: number) => get().cards.filter((card) => card.world === world),
	addWorld: () => {
	...
	removeLastWorld: () => {
		...
		set((state) => ({
			worlds: state.worlds
				...
				.map((world) => ({ ...world, id: world.id > id ? world.id - 1 : world.id })),
	...
	setWorld: (id: number) => {
		return (world: Partial<World>) => set((state) => ({
...
function createCard(world: number, position: number, id: number, rarity: CardRarity): CardStat {
	return {
		id,
