...
function TargetLine() {
  ...
  const handleMouseMove = (event: MouseEvent) => {
    ...
    const rect = element.getBoundingClientRect();
		...
		const distanceAndAngle = calculateDistanceAndAngle({ x: rect.left, y: rect.top }, { x: event.pageX, y: event.pageY });
		setAngle(distanceAndAngle.angle)
		setDistance(distanceAndAngle.distance)
  };

	const calculateDistanceAndAngle = (position: Point, mousePosition: Point): DistanceAndAngle => {
	...
	return (
		<div className="absolute top-0 left-1/2 -translate-x-2/4">
			...
			<div
				...
				style={{ transform: `translateX(-50%) rotate(${angle}deg)`, height: `${(distance)*1}px`, boxShadow: "0px 0px 2px 2px rgba(0,0,0,0.75)" }}
			>
			</div>
		</div>
	)
